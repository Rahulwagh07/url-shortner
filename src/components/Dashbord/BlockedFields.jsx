...
const { DELETE_BLOCKED_DOMAINS_API, DELETE_BLOCKED_WORDS_API, ADD_BLOCKED_DOMAINS_WORDS_API } = globalVariablesEndpoints;
...
function BlockedFields({ blockedDomains, blockedWords, setBlockedDomains, setBlockedWords }) {
    ...
    const handleDeleteBlockedDomains = async () => {
        if (selectedBlockedDomains.length > 0) {
            ...
            toast.success('Select at least 1 domain to delete');
        }
    };
    ...
    const handleDeleteBlockedWords = async () => {
        if (selectedBlockedWords.length > 0) {
            ...
            toast.success('Select at least 1 blocked words to delete');
        }
    };
      ...
      <div>
        ...
        <div className="flex items-center mt-2">
            <input
              ...
              className="border border-gray-300 rounded-md px-2 py-1 mr-2"
            />
            <button onClick={handleAddBlockedWord} className="bg-black text-white rounded-md px-4 py-2">
                Add
            </button>
        </div>
        <ul className="grid grid-cols-4 gap-2">
